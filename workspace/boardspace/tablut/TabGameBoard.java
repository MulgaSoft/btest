...
{	static final boolean debug = false;
    ...
    public TabGameBoard(String init) // default constructor
    {
	...
	{ TabGameBoard dup = new TabGameBoard(gameType()); 
	  dup.clone(this);
	  return(dup); 
   	}
    public void copyFrom(BoardProtocol b) { clone((TabGameBoard)b); }
	...
	{ int col = c.col-'A';
	  ...
	  if((col==center)&&(row==center)) { c.flagArea = Flagship_Owns_Center; }
    ...
    {	StringTokenizer tok = new StringTokenizer(gamespec);
    	...
    	if(Tablut_11_INIT.equals(game)) { nCols = nRows = 11; }
    	else if(Tablut_9_INIT.equals(game)) { nCols = nRows = 9; }
    	else if(Tablut_7_INIT.equals(game)) { nCols = nRows = 7; }
        ...
        playerChip[0]=TabChip.GoldShip;
        playerChip[1]=TabChip.SilverShip;
     ...
     *  */
    public void clone(TabGameBoard from_b)
    {	super.clone(from_b);
  ...
  */ 
    ...
    {	if(win[player]) { return(true); }
    	...
    	{ G.Error("Not expecting playerchip "+playerChip[player]);
    	return(false);
    	}
     }
    ...
    {	TabChip old = c.topChip();
    	...
    	else { G.Error("Unexepected old chip "+old);}
    	...
    	else { G.Error("Unexepected new chip "+newc);}
    	}
    	return(old);
    }
    //
...
{ 
	...
	{   for(int i=0;i<pools.length;i++)
		...
		G.Error("location "+src+" not found");
    ...
    {	Tabmovespec m = (Tabmovespec)mm;
		...
		case MOVE_SWAP:	// swap colors with the other player
			...
			break;
        case MOVE_DONE:
        	doCaptures(droppedDest);	// save capture info fo undo
         	doDone();
            break;
       ...
       case MOVE_RESET:
        	...
        	case REARRANGE_GOLD_STATE:
        		...
        		{
        			m.op = MOVE_DROPB;
        			m.to_col = pickedSource.col;
        			m.to_row = pickedSource.row;
        			for(int i=0; i<pools.length; i++) 
        				{ if (pools[i].topChip()==pickedObject) { m.object = pools[i].rackLocation(); break; }
        				}
        		}
        		unPickObject();
        		acceptPlacement();
    ...
    {
    	if((cell!=null) && cell.flagArea) { return(true); }	// only the flagship can enter
    ...
    {	int idx=0;
    	...
    	{
    	for(int dir = 1;dir<source.adjacent.length; dir+=2)
    	{	TabCell adj = source.exitTo(dir);
    		if(contentsOwnedBy(nextPlayer[whoseTurn],adj))
    		{
    		TabCell opp = adj.exitTo(dir);
    		if( contentsAssistCapture(whoseTurn,opp) && ((opp.chip!=TabChip.GoldFlag)||Flagship_Can_Capture))
    		{ 	if(Flagship_Four_Sided_Capture && (adj.chip==TabChip.GoldFlag)) 
    			{	//  the other two sides have to be surround too
    				...
    				if((adj1==null) || contentsAssistCapture(whoseTurn,adj1))
    				{
    					TabCell adj2 = adj.exitTo(dir+6);
    					if((adj2==null) || contentsAssistCapture(whoseTurn,adj2))
    					{ temp[idx++]=adj;
    					}
    				}
    			}
    ...
    {	
    	...
    	{	TabCell cap = tempDests[i];
    		...
    		int dy = 1+ (cap.row-source.row);
       		...
       		switch(dx+dy*3)
    		{ default: G.Error("Not expecting "+(dx+dy*3));
	...
	switch (board_state)
		...
		case PLAY_OR_SWAP_STATE:
			return(cellContentsOk 
					&& ((pickedSource==null)?true:(movingObjectDests().get(c)==c)));
 ...
 /** assistance for the robot.  In addition to executing a move, the robot
    ...
    */
    public void RobotExecute(Tabmovespec m)
    {
        ...
        if (Execute(m))
        {
            ...
            {	if(m.op==MOVE_MOVE) 
            	{ m.capture_mask = doCaptures(getCell(m.to_col,m.to_row));
   ...
   //
    ...
    public void UnExecute(Tabmovespec m)
    {
        ...
        switch (m.op)
        {
        case MOVE_DROPB:	// robot doesn't use this
        case MOVE_START:
        case MOVE_PICK:
        case MOVE_DROP:
        case MOVE_RESET:
		...
		{ totalSweepScore += cell.sweep_score;
		  if((cell.sweep_counter==generation) && (cell.chip==null))
			{	// tag movable cells as gen +1
				for(int direction = 1;direction<cell.adjacent.length; direction+=2)
